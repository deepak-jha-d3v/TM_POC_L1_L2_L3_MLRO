...
## Full case lifecycle (L1 → L2 → L3 → MLRO)

This build extends the L1-only POC to the complete, simplified AML case
lifecycle: L1 Review → L2 Investigation → L3 Review → Compliance/MLRO SAR/STR
decision. L1 auto-closes false positives on SARA.ai's own recommendation;
only genuinely escalated cases reach L2, then L3 (which reviews L2's work and
forwards genuine cases to MLRO), then MLRO for the final filing decision.

The lifecycle is implemented in **`workflow_engine.py`** and rendered as a new
**"Case Lifecycle"** section in the workspace. At every stage SARA.ai works the
checklist and can advance the case autonomously, but a **human-in-the-loop
sign-off is recorded on every stage**, and a **blocking human authorisation is
mandatory before any SAR/STR is filed**. See `CASE_LIFECYCLE.md` for the full
design and process-map mapping, and `CASE_LIFECYCLE_FLOW.png` for the diagram.
...
python3 generate_investigation_data.py  # -> output/investigation_data.json  (now also attaches the case lifecycle)
python3 build_investigation_ui.py       # -> output/case_review_ui.html       (full workspace incl. lifecycle)
```

To inspect the lifecycle on its own: `python3 workflow_engine.py` writes
`output/case_workflow.json` and prints the outcome distribution.
...
**This revision's major changes** (see `ARCHITECTURE.md` for the updated
end-to-end flow diagram):
1. **AI now makes the full L1 decision autonomously** — Escalate to L2 or
   Close as False Positive — using the rule-based score, red-flag weight,
   screening results, and past AML/SAR history. A human is only pulled in
   when the AI's own signals are contradictory (conflicting evidence, a
   sharp AI score adjustment, or a borderline confidence score); an analyst
   can still explicitly override any autonomous decision.
2. **RFI removed from L1** — "Request More Info" is no longer a valid L1
   outcome anywhere (AI copilot, guardrails, or the disposition UI). L1 is a
   binary call.
3. **Two separate audit trails** — autonomous AI decisions write to
   `AI_Autonomous_Decisions_Log.csv`; human decisions (contradiction cases
   and overrides) write to `L1_Audit_tracker.csv`.
4. **New Account Operations section** — device IDs, IP addresses, login
   times, and login countries per customer (`data/device_sessions.csv`),
   with anomalous-location logins highlighted.
5. **New User/Entity section** — the jurisdiction a customer/entity is
   actually registered in, separate from the country they transact from; a
   mismatch is flagged as a mild red flag.
6. **Past AML History** (renamed from "AML History") now also shows whether
   a SAR has previously been filed against the customer/entity, with case
   details.
7. **Zelle** added as a fiat transaction channel alongside card/ACH/wire/cash.
8. **Transaction pivot table** added to the Transaction Analysis section —
   pivot the 12-month history by channel/direction/country/risk
   indicator/purpose, with count/sum/average metrics.
9. **Fixed a risk-mismatch bug**: `overall_risk_score` in the Customer Risk
   Snapshot previously used only the single highest bundled alert's raw
   rule score, ignoring the customer's actual KYC risk rating and any AI
   score adjustment — so a "low risk" customer with one loud alert could
   show a score that visually contradicted their own risk-rating badge, and
   multiple bundled alerts didn't compound the score at all. It now blends
   a KYC risk-rating floor, the highest AI-adjusted score across bundled
   alerts, and a small per-additional-alert bump.
