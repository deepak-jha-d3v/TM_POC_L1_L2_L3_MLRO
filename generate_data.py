...
def make_customers():
    ...
    for i in range(N_CUSTOMERS):
        ...
        # Registered jurisdiction is usually the same as the transacting country, but a
        # slice of higher-risk customers are registered in a different jurisdiction than
        # where they actually transact (e.g. an offshore-registered entity) -- this is a
        # deliberate, mild red-flag signal surfaced in the User/Entity section.
        if risk == "high" and random.random() < 0.35:
            registered_jurisdiction = random.choice(
                [c for c in (LOW_RISK_COUNTRIES + HIGH_RISK_COUNTRIES) if c != country])
        else:
            registered_jurisdiction = country
        customers.append({
            "customer_id": cust_id,
...
DEVICE_TYPES = ["desktop_web", "ios_app", "android_app"]


def make_device_sessions(customers_df):
    """Synthetic login/device activity per customer for the Account Operations
    section: device IDs, IPs, login times, and login countries. A small slice
    of customers get an injected impossible-travel / new-device pattern
    (rapid logins from two different countries, or a brand-new device
    immediately before a large transaction window) as a mild behavioural red
    flag independent of the transaction-based rules."""
    sessions = []
    anomaly_pool = set(customers_df.sample(frac=0.2, random_state=7)["customer_id"])
    for _, cust in customers_df.iterrows():
        cust_id = cust["customer_id"]
        home_country = cust["registered_jurisdiction"]
        n_devices = random.randint(1, 3)
        device_ids = [f"DEV-{uuid.uuid4().hex[:8].upper()}" for _ in range(n_devices)]
        n_sessions = random.randint(8, 20)
        for _ in range(n_sessions):
            device_id = random.choice(device_ids)
            login_country = home_country
            if cust_id in anomaly_pool and random.random() < 0.25:
                # a foreign login inconsistent with the customer's registered jurisdiction
                login_country = random.choice(
                    [c for c in (LOW_RISK_COUNTRIES + HIGH_RISK_COUNTRIES) if c != home_country])
            sessions.append({
                ...
                "login_timestamp": rand_date(START_DATE, END_DATE).isoformat(),
                "login_country": login_country,
                "is_new_device": device_id == device_ids[-1] and n_devices > 1,
            })
    return pd.DataFrame(sessions).sort_values(["customer_id", "login_timestamp"]).reset_index(drop=True)
...
def normal_transactions_for_account(account_id, customer, txns):
    ...
    for _ in range(n):
        ...
        txns.append(base_txn(account_id, customer, rand_date(START_DATE, END_DATE), amt,
                              ...
                              random.choice(["card", "ach", "wire", "zelle"]),
...
if __name__ == "__main__":
    ...
    device_sessions_df = make_device_sessions(customers_df)
