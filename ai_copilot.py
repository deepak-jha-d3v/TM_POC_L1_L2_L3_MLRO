...
4. Recommend one suggested_action: "close_false_positive" or "escalate_to_l2". \
There is no "request more information" outcome at L1 -- L1's job is a binary \
call: either the evidence supports closing this as a false positive, or it \
does not and must go to L2 for deeper investigation. If you are genuinely \
unsure, default to "escalate_to_l2" rather than closing.
...
VALID_ACTIONS = {"close_false_positive", "escalate_to_l2"}
...
def apply_guardrails(alert: dict, result: dict) -> dict:
    ...
    if result.get("suggested_action") not in VALID_ACTIONS:
        triggered.append(f"suggested_action {result.get('suggested_action')!r} not a recognized action; defaulted to escalate_to_l2 (L1 has no request-more-info outcome, so the safe default is escalation, not closure)")
        result["suggested_action"] = "escalate_to_l2"
...
def mock_response(alert: dict) -> dict:
    ...
    # L1 has no "request more info" outcome -- it's a binary call. 50 is the
    # midpoint of the 0-100 scale; anything at or above it escalates rather
    # than closes, since the safer default under uncertainty is escalation.
    action = "close_false_positive" if final_score < 50 else "escalate_to_l2"
